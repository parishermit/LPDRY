...
LABEL_DICT = {
	'chuan':0, 'e':1, 'gan':2, 'gan1':3, 'gui':4, 'gui1':5, 'hei':6, 'hu':7, 'ji':8,
    'jin':9, 'jing':10, 'jl':11, 'liao':12, 'lu':13, 'meng':14, 'min':15, 'ning':16,
    'qing':17,'qiong':18, 'shan':19, 'su':20, 'sx':21, 'wan':22, 'xiang':23, 'xin':24,
    'yu':25, 'yu1':26, 'yue':27, 'yun':28, 'zang':29,'zhe':30
}
...
# 1. 加载数据集
# 参数：数据集所在目录位置： dir_path
# 返回值：样本的特征矩阵、标签向量，features,labels
def load_data(dir_path):
    data = []
    labels = []
    # 获取数据集目录下的所有的子目录并逐一遍历
...
def train():
    ...
    print("模型保存到:", MLP_ENU_MODEL_PATH)
