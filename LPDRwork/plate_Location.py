...
def verify_plate_by_size(contour):
    ...
    # box = np.int0(box)
    # tmp = cv.drawContours(img.copy(), [box], 0, (0, 255, 0), 2)
    # showImg(tmp, "angle: " + str(rect[2]))
    w, h = rect[1]
    w = int(w)
    h = int(h)
    minRectArea = w * h
    ...
    if ratio < 1:
        ratio = 1 / ratio
    print("ratio: " + str(ratio) + " minRectArea: " + str(minRectArea) + " w: " + str(w) + " h: " + str(h), 'angle: ' + str(rect[2]))
    MIN_AREA = 44 * 14
...
def plate_locate1(plateImage):
    originImg = plateImage
    originImg, edgedImg = preprocess(originImg)
    contours = findContours(edgedImg)
    plates = []
    for contour in contours:
